...
只在「远端被控对象」和「本地状态估计」之间插一层可达性判定：
    ...
    远端 env  --(每步以概率 p 丢弃、时延 d 步)-->  本地 StateTracker
...
3. **观测本身不因信道而失真**（不量化、不加噪、不真正延迟到达的内容）。
   信道只决定"这一步是否被本地看见"以及维持多大的信息年龄（AoI）。
   ★ 这是本模块的**诚实边界**：真正的"陈旧"由 StateTracker 的 age 记账承担，
   而不是靠改观测值假装的。

AoI 两个分量（直接对应讲座①吴泳澎的拆法）
-----------------------------------
- `transmission_age`：自上一次成功接收起的步数 → **传输信息年龄**
