...
# 10) 自检（33 项，不依赖 pytest；每个用例对应一次真实踩过的坑）
...
自检 ⑫–⑮（共 44 项）覆盖：增益闭式解、稳态=解析解、n_tx/E[age] 闭式解精确匹配、
...
自检 ⑯–⑳（共 44 项）覆盖：GE 解析量、无记忆点不是 L=1、条件年龄恒为 Geom(β)、
...
- [x] ★ **44 项自检测试**（不依赖 pytest；每个用例对应一次真实踩过的坑）
