...
自检 ⑫–⑮（共 50 项）覆盖：增益闭式解、稳态=解析解、n_tx/E[age] 闭式解精确匹配、
...
自检 ⑯–⑳（共 50 项）覆盖：GE 解析量、无记忆点不是 L=1、条件年龄恒为 Geom(β)、
...
⚠ 幅度只有 4%，**引用时必须标注量级**，不得写成"应该拉长周期"的一般结论。
...
- [x] ★ **50 项自检测试**（不依赖 pytest；每个用例对应一次真实踩过的坑）
